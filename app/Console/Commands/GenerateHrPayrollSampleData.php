...
class GenerateHrPayrollSampleData extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'hr-payroll:generate-sample-data';
     ...
     * @var string
     */
    protected $description = 'Generate sample data for HR Payroll module';
    ...
    {
        $this->info('Generating HR Payroll sample data...');
        
        // Ask for confirmation
        if (!$this->confirm('This will create sample data in your database. Continue?')) {
        ...
        $this->info('Found ' . $users->count() . ' users to create sample data for.');
        
        // 1. Create Employee HR Details
        $this->createEmployeeHrDetails($users);
        
        // 2. Create Payroll Periods
        $payrollPeriods = $this->createPayrollPeriods();
        
        // 3. Create Payroll Records
        $this->createPayrollRecords($users, $payrollPeriods);
        
        // 4. Create Time & Attendance
        $this->createTimeAttendance($users);
        
        // 5. Create Leave Requests
        $this->createLeaveRequests($users);
        
        // 6. Create Leave Balances
        $this->createLeaveBalances($users);
        
        // 7. Create Tax Settings
        $this->createTaxSettings();
        
        // 8. Create Deduction Settings
        $deductionSettings = $this->createDeductionSettings();
        
        // 9. Create Employee Deductions
        $this->createEmployeeDeductions($users, $deductionSettings);
        
        // 10. Create Payroll History Logs
        $this->createPayrollHistoryLogs($users, $payrollPeriods);
        
        $this->info('✅ HR Payroll sample data generated successfully!');
        $this->info('You can now visit: https://app.fixitautoservices.com/hr-payroll');
    }
    
    private function createEmployeeHrDetails($users)
    {
        $this->info('Creating employee HR details...');
        ...
        DB::table('employee_hr_details')->insert($records);
        $this->info('Created ' . count($records) . ' employee HR details');
    }
    
    private function createPayrollPeriods()
    {
        $this->info('Creating payroll periods...');
        ...
        for ($i = 1; $i <= 3; $i++) {
            ...
            $periods[] = [
                ...
                'employee_count' => 6, // Based on user count
                'processed_by' => 1,
        ...
        $this->info('Created ' . count($periods) . ' payroll periods');
        
        return DB::table('payroll_periods')->get();
    }
    
    private function createPayrollRecords($users, $payrollPeriods)
    {
        $this->info('Creating payroll records...');
        ...
        foreach ($payrollPeriods as $period) {
            foreach ($users as $user) {
                ...
                $records[] = [
                    ...
                    'status' => in_array($period->status, ['draft', 'calculated', 'approved', 'paid', 'cancelled']) ? 
                    ($period->status === 'processing' ? 'calculated' : $period->status) : 'draft',
        ...
        $this->info('Created ' . count($records) . ' payroll records');
    }
    
    private function createTimeAttendance($users)
    {
        $this->info('Creating time & attendance records...');
        ...
        DB::table('time_attendance')->insert($records);
        $this->info('Created ' . count($records) . ' time & attendance records');
    }
    
    private function createLeaveRequests($users)
    {
        ...
        foreach ($users as $user) {
            for ($j = 1; $j <= rand(1, 3); $j++) {
                ...
                $records[] = [
                    ...
                    'reason' => 'Sample ' . $leaveTypes[array_rand($leaveTypes)] . ' leave request',
        ...
        DB::table('leave_requests')->insert($records);
        $this->info('Created ' . count($records) . ' leave requests');
    }
    
    private function createLeaveBalances($users)
    {
        $this->info('Creating leave balances...');
        ...
        DB::table('leave_balances')->insert($records);
        $this->info('Created ' . count($records) . ' leave balances');
    }
    
    private function createTaxSettings()
    {
        $this->info('Creating tax settings...');
        ...
        DB::table('tax_settings')->insert($records);
        $this->info('Created ' . count($records) . ' tax settings');
    }
    
    private function createDeductionSettings()
    {
        $this->info('Creating deduction settings...');
        ...
        DB::table('deduction_settings')->insert($records);
        $this->info('Created ' . count($records) . ' deduction settings');
        
        return DB::table('deduction_settings')->get();
    }
    
    private function createEmployeeDeductions($users, $deductionSettings)
    {
        $this->info('Creating employee deductions...');
        ...
        DB::table('employee_deductions')->insert($records);
        $this->info('Created ' . count($records) . ' employee deductions');
    }
    
    private function createPayrollHistoryLogs($users, $payrollPeriods)
    {
        $this->info('Creating payroll history logs...');
        ...
        DB::table('payroll_history_logs')->insert($records);
        $this->info('Created ' . count($records) . ' payroll history logs');
    }
}