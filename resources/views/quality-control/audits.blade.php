@extends('layouts.app')

@section('title', 'Quality Audits')

@section('content')
<div class="container-fluid">
    <!-- Header -->
    <div class="row mb-4">
        <div class="col">
            <h1 class="h3 mb-0">Quality Audits</h1>
            <p class="text-muted">Review inspection audits and their results</p>
        </div>
        <div class="col-auto">
            <a href="{{ route('quality-control.audits.create') }}" class="btn btn-primary">
                <i class="fas fa-plus"></i> New Audit
            </a>
        </div>
    </div>

    <!-- Filters -->
    <div class="card shadow mb-4">
        <div class="card-body">
            <form method="GET" action="{{ route('quality-control.audits.index') }}" class="row g-3">
                <div class="col-md-3">
                    <label class="form-label">Status</label>
                    <select name="status" class="form-select">
                        <option value="">All Statuses</option>
                        <option value="passed" {{ request('status') == 'passed' ? 'selected' : '' }}>Passed</option>
                        <option value="failed" {{ request('status') == 'failed' ? 'selected' : '' }}>Failed</option>
                        <option value="pending" {{ request('status') == 'pending' ? 'selected' : '' }}>Pending</option>
                    </select>
                </div>
                <div class="col-md-3">
                    <label class="form-label">Technician</label>
                    <select name="technician_id" class="form-select">
                        <option value="">All Technicians</option>
                        @foreach($technicians as $tech)
                            <option value="{{ $tech->id }}" {{ request('technician_id') == $tech->id ? 'selected' : '' }}>{{ $tech->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-3">
                    <label class="form-label">Auditor</label>
                    <select name="auditor_id" class="form-select">
                        <option value="">All Auditors</option>
                        @foreach($auditors as $aud)
                            <option value="{{ $aud->id }}" {{ request('auditor_id') == $aud->id ? 'selected' : '' }}>{{ $aud->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-3 d-flex align-items-end">
                    <button type="submit" class="btn btn-secondary me-2"><i class="fas fa-filter"></i> Filter</button>
                    <a href="{{ route('quality-control.audits.index') }}" class="btn btn-outline-secondary">Reset</a>
                </div>
            </form>
        </div>
    </div>

    <!-- Audits Table -->
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Audits ({{ $audits->total() }})</h6>
        </div>
        <div class="card-body">
            @if($audits->count() > 0)
                <div class="table-responsive">
                    <table class="table table-hover align-middle">
                        <thead>
                            <tr>
                                <th>Audit #</th>
                                <th>Title</th>
                                <th>Technician</th>
                                <th>Auditor</th>
                                <th>Date</th>
                                <th>Score</th>
                                <th>Status</th>
                                <th>Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach($audits as $audit)
                                <tr>
                                    <td>{{ $audit->audit_number ?? 'N/A' }}</td>
                                    <td>
                                        <a href="{{ route('quality-control.audits.show', $audit->id) }}" class="fw-bold text-decoration-none">
                                            {{ $audit->title }}
                                        </a>
                                    </td>
                                    <td>{{ $audit->technician->name ?? 'N/A' }}</td>
                                    <td>{{ $audit->auditor->name ?? 'N/A' }}</td>
                                    <td>{{ $audit->audit_date ? \Carbon\Carbon::parse($audit->audit_date)->format('M d, Y') : 'N/A' }}</td>
                                    <td>{{ $audit->percentage_score ?? 0 }}%</td>
                                    <td>
                                        @php
                                            $statusColor = match($audit->status) {
                                                'passed' => 'success',
                                                'failed' => 'danger',
                                                'in_progress' => 'warning',
                                                default => 'secondary',
                                            };
                                        @endphp
                                        <span class="badge bg-{{ $statusColor }}">{{ ucfirst(str_replace('_', ' ', $audit->status ?? 'pending')) }}</span>
                                    </td>
                                    <td>
                                        <a href="{{ route('quality-control.audits.show', $audit->id) }}" class="btn btn-sm btn-outline-secondary">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                </div>
                <div class="d-flex justify-content-center mt-3">
                    {{ $audits->links() }}
                </div>
            @else
                <div class="text-center py-5">
                    <i class="fas fa-clipboard-check fa-3x text-muted mb-3"></i>
                    <p class="text-muted">No audits found.</p>
                </div>
            @endif
        </div>
    </div>
</div>
@endsection
